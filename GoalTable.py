...
conn.execute("CREATE TABLE IF NOT EXISTS GOAL(ID INTEGER PRIMARY KEY AUTOINCREMENT ,GNAME TEXT NOT NULL,"+
					"GTIME INTEGER NOT NULL, GSTATUS INTEGER NOT NULL DEFAULT 0, T1NAME TEXT NOT NULL, T1TIME INTEGER NOT NULL, T1STATUS INTEGER NOT NULL  DEFAULT 0, T2NAME TEXT NOT NULL,"
...
try:
	conn.execute("INSERT INTO GOAL(GNAME, GTIME, T1NAME, T1TIME, T2NAME, T2TIME, T3NAME, T3TIME) VALUES ('"+goalname+" ', '"+goaltime+"', '"+t1name+"', '"+t1time+"', '"+t2name+"', '"+t2time+"', '"+t3name+"', '"+t3time+"');")
	conn.commit()
	print ("Done!!")
except:
	print ("Error!")
